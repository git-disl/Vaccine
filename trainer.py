...
class DecentralizedTrainer(Trainer):
    ...
    def topk(self, layer_grad):
            ...
            # Flatten the tensor to make it 1D
            grad_squared_flat = grad_squared.flatten()
            dense_num = int(torch.numel(grad_squared_flat) * self.args.density)
            # Get the indices of the top-K elements
            topk_indices = torch.topk(grad_squared_flat, k=dense_num, largest=True)[1]
            # Create a mask with zeros and ones, where ones correspond to the top-K elements
            mask = torch.zeros_like(grad_squared_flat)
    ...
    def training_step(
        ...
        def step():
            ...
            if self.do_grad_scaling:
                ...
                self.accelerator.backward(loss)
            return loss 
        if self.num_step% self.args.check_periodicity:
            loss = step()
            leaf_modules_with_grad = get_leaf_modules_with_grad(model)
            self.masks  = []
            for module in leaf_modules_with_grad:
                # print(module.q_proj.lora_A["default"])
                mask = self.topk( torch.square(module.weight.grad.detach().clone()))
                self.masks +=[mask]
        loss = step()
        leaf_modules_with_grad = get_leaf_modules_with_grad(model)
        print(get_leaf_modules_with_grad)
        index=0
        for module in leaf_modules_with_grad:
            # print(module.q_proj.lora_A["default"])
            module.weight.grad*= self.masks[index]
            index+=1
        self.num_step+=1
        return loss.detach() / self.args.gradient_accumulation_steps